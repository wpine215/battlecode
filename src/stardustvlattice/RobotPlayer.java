...
//                scoutLocalQuadrant();
    ...
    static void runLandscaper() throws GameActionException {
        if (rc.getRoundNum() >= 6 && enemyHQ == null) {
            Transaction rb = communication.getLastRebroadcast();
            MapLocation tryEnemyHQ = communication.getEnemyHQFromRebroadcast(rb);
            System.out.println("ENEMY HQ FROM BROADCAST IS " + tryEnemyHQ);
            if (!Pathfinding.locIsNull(tryEnemyHQ)) {
...
//                        if (!(otherBot != null && otherBot.team == rc.getTeam() && otherBot.getType() != RobotType.DESIGN_SCHOOL)) {
        ...
        if (rc.canSenseLocation(localHQ)) {
            if (!tryBuildWall()) {
                landscaperState = LandscaperState.LATTICE;
                runLandscaper();
            }
        } else {
        ...
        if (!latticeWallBuilt) {
            ...
            if (!pathfinding.travelTo(next_coordinate)) {
                ...
                if (rc.getLocation().x == next_coordinate.x && rc.getlocation().y == next_coordinate.y) {
                    ...
                    Direction digDirtFrom = getLatticeDigDir();
                    if (digDirtFrom != null && rc.canDigDirt(digDirtFrom)) {
                        rc.digDirt(digDirtFrom);
                    }
                }
                else if (next_coordinate.x + next_coordinate.y > 3) {
                    // if blocked and outside of wall, pick a location to start latticing and start doing so
                }
            // if blocked to movement and inside the wall, iterate from that index
